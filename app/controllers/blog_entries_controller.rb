...
uses_tiny_mce(:options => AppConfig.default_mce_options, :only => [:new, :edit, :index, :create])
  ...
  def create
    ...
    if @blog_entry.save
      ...
      if !@project.members.empty? && !@project.blog_post_collaborators.empty?
        @project.blog_post_collaborators.each do |user|
        Mailer.send_later(:deliver_add_entry_to_blog_collaborator,@project,current_profile,user.display_name,[user.email]-[current_profile.email])
           end
      end
      if !@project.followers.empty? && !@project.blog_post_followers.empty?
       @follow=@project.blog_post_followers
       @prof=@project.profiles
       followers_blog_mails= @follow - @prof
       followers_blog_mails.each do |user|
           Mailer.send_later(:deliver_add_entry_to_blog_followers,@project,current_profile,user.display_name,[user.email]-[current_profile.email])
      end
      end
    else
      render :action => "new"
